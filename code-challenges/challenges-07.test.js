...
if (string.replace(regex1)){
return string.replace(regex1, 'zero');
}
;// Solution code here...
}
...
const addOne = (arr) => {
  let addOneArr =arr.map((item)=>{
    return item=item+1;// Solution code here...
});
return addOneArr
}
...
const addQuestion = (arr) => {
  let addQuestion =arr.map((item)=>{
    return `${item}?`;// Solution code here...
});
return addQuestion // Solution code here...
}
